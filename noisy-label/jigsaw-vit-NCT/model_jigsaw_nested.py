...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(embed_dim): 
            tmp = torch.cuda.FloatTensor(1, embed_dim).fill_(0)
            tmp[:, : (i + 1)] = 1
    ...
    def forward_cls(self, x, k=None, dist=None) : 
        # embed patches
        x = self.patch_embed(x)
        ...
        if k is None:
            k = np.random.choice(range(len(self.mask_feat_dim)), p=dist)

        mask_k = self.mask_feat_dim[k]
        masked_feat = feat * mask_k
        
        x = self.head(masked_feat)
        return x, feat

    def forward(self, x_jigsaw, x_cls, mask_ratio, k=None, dist=None) :
        pred_jigsaw, targets_jigsaw = self.forward_jigsaw(x_jigsaw, mask_ratio)
        pred_cls, _ = self.forward_cls(x_cls, mask_ratio, k, dist)
...
def mae_vit_small_patch4(nb_cls, **kwargs):
    model = MaskedAutoencoderViT(nb_cls,
                                 img_size=32,
                                 patch_size=4,
                                 embed_dim=384,
...
def mae_vit_small_patch8(nb_cls, **kwargs):
    model = MaskedAutoencoderViT(nb_cls,
                                 img_size=64,
                                 patch_size=8,
                                 embed_dim=384,
...
def vit_small_patch16_224(nb_cls, **kwargs):
    model = MaskedAutoencoderViT(nb_cls,
                                 img_size=224,
                                 patch_size=16,
                                 embed_dim=768,
...
def mae_vit_base_patch8(nb_cls, **kwargs):
    model = MaskedAutoencoderViT(nb_cls,
                                 img_size=64,
                                 patch_size=8,
                                 embed_dim=768,
...
def create_model(arch, nb_cls) :     
    if arch == 'vit_tiny_patch16' :
        return mae_vit_tiny_patch16(nb_cls)
    elif arch == 'vit_small_patch4':
        return mae_vit_small_patch4(nb_cls)
    elif arch == 'vit_small_patch8' :
        return mae_vit_small_patch8(nb_cls)
    elif arch == 'vit_small_patch16' : 
        return mae_vit_small_patch16(nb_cls)
    elif arch == 'vit_base_patch8' :
        return mae_vit_base_patch8(nb_cls)
        ...
        return vit_small_patch16_224(nb_cls)
    
if __name__ == '__main__':
    
    
    net = create_model(arch = 'vit_small_patch4', nb_cls = 10) 
    net = net.cuda()
    img = torch.cuda.FloatTensor(6, 3, 32, 32)
    mask_ratio = 0.75
    with torch.no_grad():
