...
def plot_lr(warmup_iter, total_iter, max_lr, min_lr, out_dir) :
    
    iter_list = np.arange(total_iter)
    lr_list = [update_lr(e, warmup_iter, total_iter, max_lr, min_lr) for e in iter_list]
    plt.plot(iter_list, lr_list)
    plt.xlabel('Iter')
    plt.ylabel('Lr')
    out_name = os.path.join(out_dir, 'lr.png')
    plt.legend(loc='best')
    plt.grid('on')
